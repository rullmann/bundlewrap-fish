...
if node.metadata.get('fish', {}).get('install_fisherman', True):
    actions['install_fisherman'] = {
        ...
        'cascade_skip': False,
        'needs': ['pkg_dnf:fish', 'pkg_dnf:curl'],
    }
    for plugin in node.metadata.get('fish', {}).get('plugins', {}):
        actions['fisher_{}'.format(plugin)] = {
            ...
            'cascade_skip': False,
            'needs': ['pkg_dnf:fish', 'action:install_fisherman', 'action:enable_fish'],
        }
    ...
    if node.metadata.get('fish', {}).get('install_fisherman', True):
        ...
        for plugin in node.metadata.get('fish', {}).get('plugins', {}):
            ...
            actions['fisher_{}_{}'.format(user, plugin)] = {
                ...
                'cascade_skip': False,
                'needs': ['pkg_dnf:fish', 'action:install_fisherman_{}'.format(user), 'action:enable_fish_{}'.format(user)],
            }
